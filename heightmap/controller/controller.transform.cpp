...
bool Controller::raytrace(vec3& point, const vec2i& mp) {
	...
	invmat.invert();
	vec4 r0 = data.view.unprojectFull(vec4((float)mp.x, (float)Controller::get().getParent()->height - mp.y, 0.0f, 1.0f), *Controller::get().getParent());
	vec4 r1 = data.view.unprojectFull(vec4((float)mp.x, (float)Controller::get().getParent()->height - mp.y, 1.0f, 1.0f), *Controller::get().getParent());
